...
def reinforce_node():
    ...
    xy_policy_var = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='xy_planepolicy')
    xy_value_var = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='xy_planevalue')

    restore_vars = policy_var + value_var + xy_policy_var + xy_value_var
    ...
    try:
        saver.restore(sess, "./Graph/model-606.cptk")
    ...
    while not rospy.is_shutdown():
        ...
        while 1 and not rospy.is_shutdown():
            ...
            if probability[0][0][1] > probability[0][0][2]:
                executed_action[0] = -1.0
            else:
                executed_action[0] = 1.0

            if probability[0][0][3] > probability[0][0][4]:
